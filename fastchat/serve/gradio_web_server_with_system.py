...
def clear_chats_history(state, request: gr.Request):
    ip = get_ip(request)
    logger.info(f"clear_history. ip: {ip}")
    if state is not None:
        if state.conv.messages[0][0] == state.conv.roles[1]:  # greeting
            state.conv.messages = state.conv.messages[:1]
        # if state.conv.messages[0][0] == state.conv.roles[0] and state.conv.messages[0][1] == "":  # greeting
        #     state.conv.messages = state.conv.messages[:2]
        else:
            state.conv.messages = []
...
def set_sys(state, model_selector, system, request: gr.Request):
    ip = get_ip(request)
    logger.info(f"set_sys. ip: {ip}. system:\n{system}")
    ...
    return (state, state.to_gradio_chatbot(), system, None) + (disable_btn,) * 6


def set_greeting(state, model_selector, greeting, request: gr.Request):
    ip = get_ip(request)
    logger.info(f"set_greeting. ip: {ip}. greeting:\n{greeting}")
    ...
    if greeting is not None and len(greeting) > 0:
        if len(state.conv.messages) > 0:
            if state.conv.messages[0][0] == state.conv.roles[1]:
                state.conv.messages[0][1] = greeting
            # if state.conv.messages[0][0] == state.conv.roles[0] and state.conv.messages[0][1] != "":
            #     state.conv.messages.insert(0, [state.conv.roles[0], ""])
            #     state.conv.messages.insert(1, [state.conv.roles[1], greeting])
            #     # state.conv.messages[0][1] = greeting
            # elif state.conv.messages[0][0] == state.conv.roles[0] and state.conv.messages[0][1] == "":
            #     state.conv.messages[1] = [state.conv.roles[1], greeting]
            else:
            #     state.conv.messages[0] = [state.conv.roles[1], greeting]
                state.conv.messages.insert(0, [state.conv.roles[1], greeting])
        else:
            # state.conv.append_message(state.conv.roles[0], "")
            state.conv.append_message(state.conv.roles[1], greeting)

    return (state, state.to_gradio_chatbot(), greeting, None) + (disable_btn,) * 6
...
"""
    ...
    with gr.Row():
        sysbox = gr.Textbox(
            max_lines=20,
            show_label=False,
            placeholder="👉 Set your agent's system and press ENTER(or press SHIFT + ENTER for new line)",
            elem_id="sys_box",
        )
        set_btn = gr.Button(value="设置", variant="primary", size="sm", scale=0.1, elem_id="set_btn")
    with gr.Row():
        greetingbox = gr.Textbox(
            show_label=False,
            placeholder="👉 Set your agent's greeting to the user and press ENTER(or press SHIFT + ENTER for new line)",
            elem_id="greeting_box",
        )
        set_greeting_btn = gr.Button(value="设置", variant="primary", size="sm", scale=0.1, elem_id="set_greeting_btn")
    with gr.Row():
        textbox = gr.Textbox(
            show_label=False,
    ...
    with gr.Row() as button_row:
        upvote_btn = gr.Button(value="👍", interactive=False)
        downvote_btn = gr.Button(value="👎", interactive=False)
        flag_btn = gr.Button(value="⚠️  Flag", interactive=False)
        regenerate_btn = gr.Button(value="🔄  Regenerate", interactive=False)
        clear_btn = gr.Button(value="🗑️  Clear all", interactive=False)
        clear_chats_btn = gr.Button(value="🗑️ Clear chats context only", interactive=True)
    ...
    btn_list = [upvote_btn, downvote_btn, flag_btn, regenerate_btn, clear_btn, clear_chats_btn]
    upvote_btn.click(
    ...
    clear_chats_btn.click(clear_chats_history, state, [state, chatbot, textbox, imagebox] + btn_list)
    ...
    sysbox.submit(
        set_sys, [state, model_selector, sysbox], [state, chatbot, sysbox] + btn_list
    )
    set_btn.click(
        set_sys,
        [state, model_selector, sysbox],
        [state, chatbot, sysbox] + btn_list,
    )

    greetingbox.submit(
        set_greeting, [state, model_selector, greetingbox], [state, chatbot, greetingbox] + btn_list
    )
    set_greeting_btn.click(
        set_greeting,
        [state, model_selector, greetingbox],
        [state, chatbot, greetingbox] + btn_list,
    )
